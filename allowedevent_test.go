...
func TestAllowedEventNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.AllowedEvents.New(context.TODO(), oursprivacy.AllowedEventNewParams{
		Name:           "name",
...
func TestAllowedEventUpdateWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.AllowedEvents.Update(
		context.TODO(),
		"id",
		oursprivacy.AllowedEventUpdateParams{
			DestinationIDs: []string{"string"},
